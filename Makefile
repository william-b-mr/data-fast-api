...
test_kitt:
	@echo "🧪 computing and saving your progress at 'tests/api/test_output.txt'..."
	@pytest tests/api -c "./tests/pytest_kitt.ini" 2>&1 > tests/api/test_output.txt
	@echo "📌 Please git add --all; git commit; ggpush to track progress on kitt"

test_api_root:
	pytest tests/api -k 'test_root' --asyncio-mode=strict -W "ignore"

test_api_predict:
	pytest tests/api -k 'test_predict' --asyncio-mode=strict -W "ignore"
